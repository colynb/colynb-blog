...
docpadConfig = {
	...
	collections:
	    ...
	    projects: (database) ->
	        database.findAllLive({relativeOutDirPath: 'projects'},[{title: 1}]).on "add", (model) ->
	            model.setMetaDefaults({layout: "project"})

	    pages: (database) ->
	        database.findAllLive({relativeOutDirPath: 'pages'}).on "add", (model) ->
	            model.setMetaDefaults({layout: "page"})

	    pictures: (database) ->
	        database.findAllLive({relativeOutDirPath: 'pictures'},[{date: -1}]).on "add", (model) ->
